...
module.exports = cds.service.impl(async function () {
  ...
  // ════════════════════════════════════════════════════════════════════
  // Helper: PO Status → Criticality 변환
  // ════════════════════════════════════════════════════════════════════
  function getStatusCriticality(status) {
    switch (status) {
      case 'Draft':     return 0; // None
      case 'Submitted': return 2; // Warning (Orange)
      case 'Approved':  return 3; // Success (Green)
      case 'Rejected':  return 1; // Error (Red)
      case 'Received':  return 3; // Success (Green)
      default:          return 0;
    }
  }

  // ════════════════════════════════════════════════════════════════════
  // Helper: 재고 부족 Criticality 계산
  // ════════════════════════════════════════════════════════════════════
  function getInventoryCriticality(availableQty, safetyStock) {
    if (safetyStock != null && availableQty < safetyStock) return 1; // Error (Red)
    if (safetyStock != null && availableQty < safetyStock * 1.5) return 2; // Warning (Orange)
    return 3; // Success (Green)
  }
  ...
  // ════════════════════════════════════════════════════════════════════
  // PurchaseOrders - AFTER READ: Criticality 계산
  // ════════════════════════════════════════════════════════════════════
  this.after('READ', PurchaseOrders, (data) => {
    const items = Array.isArray(data) ? data : [data];
    items.forEach(item => {
      if (item) {
        item.criticality = getStatusCriticality(item.status);
      }
    });
  });

  // ════════════════════════════════════════════════════════════════════
  // Inventories - BEFORE READ: availableQty 계산 (computed field)
  // ════════════════════════════════════════════════════════════════════
  this.after('READ', Inventories, (data) => {
    ...
    items.forEach(item => {
      if (item) {
        ...
        item.criticality = getInventoryCriticality(item.availableQty, item.safetyStock);
      }
    });
  });
  ...
  this.on('submitOrder', PurchaseOrders, async (req) => {
    ...
    updated.criticality = getStatusCriticality(updated.status);
